...
Finally with ```collect(by time)``` messages will be spread evenly but in packets, not one by one. So theoretically every element of such array should be republished, which would add extra complexity.
